...
use Muliix\Http\Requests;
use Muliix\Http\Controllers\Controller;
use Muliix\Http\Controllers\Sistema\DAOGeneralController;

require_once public_path()."/plugins/PHPJasperXML/class/pchart2/class/pData.class.php";
require_once public_path()."/plugins/PHPJasperXML/class/pchart2/class/pImage.class.php";

//version 0.9d
class PHPJasperXML extends Controller {
    private $adjust=1.2;
    public $version="0.9d";
    private $pdflib;
    private $lang;
...
public $mensajeNoDatos;
    public function setMensajeNoDatos($mensajeSindatos){
        $this->mensajeNoDatos = $mensajeSindatos;
    }
...
//read level 0,Jasperreport page setting
    ...
    public function setConsulta($query){
        ...
        //$consulta = \DB::select(\DB::raw($this->sql));
        $dao = new DAOGeneralController();
        $consulta = $dao->getEjecutaConsulta($this->sql);
...
//        $this->pointer[]=array("type"=>"MultiCell","width"=>$data->reportElement["width"],"height"=>$height,"txt"=>$data->text,"border"=>$border,"align"=>$align,"fill"=>$fill,"hidden_type"=>"statictext","soverflow"=>$stretchoverflow,"poverflow"=>$printoverflow,"rotation"=>$rotation);
    ...
    public function getPrintWhenExpression($parametro){
        if(isset($this->arrayParameter)) {
            foreach($this->arrayParameter as  $v => $a) {
                if('$P{'.$v.'}' == $parametro){
                    return $a;
    ...
    public function element_image($data) {
        ...
        $parametro = "";
        $reportElement = "";
        $lmx = get_object_vars($data->children());
        if(array_key_exists('reportElement', $lmx));
        $reportElement = get_object_vars(get_object_vars($data->children())['reportElement']);
        if(array_key_exists('printWhenExpression', $reportElement));
        $parametro = $reportElement['printWhenExpression'];

        if($this->getPrintWhenExpression($parametro)=="true") {
...
//                            echo "Barcode: $code,position: $textposition <br/><br/>";
            $this->pointer[]=array("type"=>"Barcode","barcodetype"=>$barcodemethod,"x"=>$x,"y"=>$y,"width"=>$width,"height"=>$height,'textposition'=>$textposition,'code'=>$code,'modulewidth'=>$modulewidth,"elementid"=>$this->elementid);
...
//                     $out["target"]= $this->analyse_expression( $out['target'], true);
    ...
    public function outpage($out_method="I",$filename="", $BanPrint='false') {
...
//          print_r($param);
    ...
    public function element_Chart($data,$type){
         ...
         if($data->chart->chartTitle->titleExpression.""!=""){
          ...
          $chartlegend['backgroundColor']=($data->chart->chartLegend['backgroundColor'].""?:'#FFFFFF');
...
//						echo $out["hidden_type"]."<br/>";
            ...
            if(isset($this->mensajeNoDatos)){
                echo($this->mensajeNoDatos);
                exit(0);
            }else{
...
*/
    ...
    public function formatText($txt,$pattern) {
        ...
        elseif($pattern=="#,##0.00" || $pattern=="#,##0.00;-#,##0.00")
            return number_format($txt,2,".",",");
        elseif($pattern=="¤ #,##0.00" || $pattern=="¤ #,##0.00;-¤ #,##0.00")
            return number_format($txt,2,".",",") == '' ? '' :  '$ '.number_format($txt,2,".",",");
